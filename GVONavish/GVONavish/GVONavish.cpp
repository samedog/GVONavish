	...
	_In_ int       nCmdShow )
{
	...
	try {
		...
		::MessageBox( NULL, L"航路保存に失敗しました。", k_appName, MB_ICONERROR );
	}
...
static void s_popupMenu( HWND hwnd, int16_t x, int16_t y )
{
	...
	mii.dwTypeData = L"[DEBUG]自動航行を有効";
	...
	mii.dwTypeData = L"[DEBUG]航路を閉じる";
	::InsertMenuItem( popupMenu, ::GetMenuItemCount( popupMenu ), TRUE, &mii );

	mii.wID = IDM_DEBUG_INTERVAL_NORMAL;
	mii.dwTypeData = L"[DEBUG]更新間隔 - 標準";
	::InsertMenuItem( popupMenu, ::GetMenuItemCount( popupMenu ), TRUE, &mii );

	mii.wID = IDM_DEBUG_INTERVAL_HIGH;
	mii.dwTypeData = L"[DEBUG]更新間隔 - 高頻度";
	::InsertMenuItem( popupMenu, ::GetMenuItemCount( popupMenu ), TRUE, &mii );
#endif
